...
@eq-vec-chain-01 에서 $\pardifftwo{ \pmb f}{\pmb g}$는 $p \times m$ Jacovian 벡터이고
...
위의 식에서 상수벡터 $\pmb a$는 가 전치로 앞에 나타나는 표현 $\pmb a^T \pmb x$ 를 사용하면
결과 벡터 $\pmb a^T$가 행벡터로 그대로 나타나지므로 **내적의 미분 표기**로 사용할 것이다.
...
$$ \pardifftwo{\pmb A \pmb x}{\pmb x}  = \pmb A $$ {#eq-vec-diff-linear}
